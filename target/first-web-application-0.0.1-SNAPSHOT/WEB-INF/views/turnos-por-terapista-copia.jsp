...
<body>
	<div class="container">
		...
		<label>Lista de Terapistas </label>
		<select>
			    <c:forEach var="terap" items="${terapistas}">
			        <option value="${terap.legajo}" ${terap.legajo == selectedDept ? 'selected="selected"' : ''} >${terap.nombre}, ${terap.apellido} - ${terap.legajo}</option>
			    </c:forEach>
		</select>
		...
		<form action="/terapistas/turnos" method="POST">
		    ...
		    <label>Legajo Terapista </label>
		    <input type="text" id="teraplegajo" name="teraplegajo" />
